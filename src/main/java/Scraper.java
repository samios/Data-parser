...
public class Scraper {
    ...
    public static String jsonFolder="/home/vnc/Conversion/Json";
    ...
    //TODO: take only same name start files into account
            ...
            if(!Conf.getConsideredStatus().equals("null"))
            {
                if(!Conf.getConsideredStatus().contains(line.get(Integer.parseInt(p.getBenchTest().getStatus()))))
                {
                        ...
                        if(f==null) {
                            ...
                            f = new File(Conf.getDirectory() + "/Passed/" + filepath.split("/")[filepath.split("/").length - 1] );
    ...
    static public String[] getDirList(Conf c) {
        File file = new File(c.getDirectory());
